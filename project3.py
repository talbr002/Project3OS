...
#	x.writeToArchive(thefile)
	...
	for line in afile:
	    ...
	    if(lineLength > 32):
		    ...
		    self.dataEntries[dataLocation - 1].readFromArchive(dataLocation, line[(0+number):(32 + number)])
		    self.archiveEntries[location].datablocks[count] = str((location + 1))
		    count = count + 1
#		    id = id + 1
#		    print(id)
		    number += 32
		...
		dataLocation = heapq.heappop(OPEN_DATA)[0]
		print(dataLocation)
		self.dataEntries[(dataLocation)].readFromArchive((dataLocation + 1), (line[number:]) + (additionalZs * "Z"))
		self.archiveEntries[location].datablocks[count] = str(1)
#		id += 1
